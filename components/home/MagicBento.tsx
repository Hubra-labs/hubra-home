...
const DEFAULT_GLOW_COLOR = "218, 165, 32";
...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const section = gridRef.current.closest<HTMLElement>(".bento-section");
...
const getInitialCapabilities = (): RuntimeCapabilities => {
  if (typeof window === "undefined" || typeof window.matchMedia !== "function") {
    return { isTouchPrimary: false, prefersReducedMotion: false };
  }
...
const useRuntimeCapabilities = (): RuntimeCapabilities => {
  const [capabilities, setCapabilities] = useState<RuntimeCapabilities>(getInitialCapabilities);
  ...
  useEffect(() => {
    ...
    const update = () =>
      setCapabilities({
        isTouchPrimary: touchMql.matches,
...
export const MagicBento = ({
  ...
  const { isTouchPrimary, prefersReducedMotion } = useRuntimeCapabilities();
  const shouldDisableAnimations = disableAnimations || isTouchPrimary || prefersReducedMotion;

  return (
    <>
      {enableSpotlight && !shouldDisableAnimations && (
        <GlobalSpotlight enabled={enableSpotlight} glowColor={glowColor} gridRef={gridRef} spotlightRadius={spotlightRadius} />
      )}
      ...
      <div ref={gridRef} className="card-grid bento-section">
        {items.map((item, index) => {
          ...
          const body = (
            <>
              <div className="magic-bento-card__header">
                {item.icon ? <div className="magic-bento-card__icon">{item.icon}</div> : <span />}
                <div className="magic-bento-card__label">{item.label}</div>
              </div>
              {item.hero ? <div className="magic-bento-card__hero">{item.hero}</div> : null}
          ...
          return (
            <div key={`${item.label}-${index}`} className={baseClassName} style={cardStyle}>
              {body}
            </div>
          );
        })}
