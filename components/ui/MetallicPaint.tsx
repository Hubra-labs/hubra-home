...
function processImage(img: HTMLImageElement): ImageData | null {
  const MAX_SIZE = 1000;
  const MIN_SIZE = 500;
...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    if (!canvas || !gl) return;
    const side = Math.round(1000 * window.devicePixelRatio);
  ...
  return <canvas ref={canvasRef} className={className ? `paint-container ${className}` : "paint-container"} />;
}
